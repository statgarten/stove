...
```{r}
split_tmp <- goophi::trainTestSplit(
  data = cleaned_data,
  target = targetVar,
  prop = trainSetRatio,
...
imputation <- TRUE
normalization <- TRUE
nomialImputationType <- "knn" # mode(default), bag, knn
numericImputationType <- "knn" # mean(default), bag, knn, linear, lower, median, roll
...
rec <- goophi::prepForCV(data = data_train,
                         ...
                         nomialImputationType = nomialImputationType, 
...
mode <- "classification"
algo <- "LogisticR"
engine <- "stan" # glm, glmnet, liblinear, stan
...
grid_search_result <- goophi::gridSearchCV(
  rec = rec,
  model = model,
  v = as.numeric(v),
  data = data_train,
...
```{r}
names(models_list)
model_name <- "LogisticR_stan" # 원하는 모델의 이름 입력받기
```
