...
# 사용자정의 ML 모델을 생성합니다
model2 <- goophi::mlp_phi(engine = engine,
                                  mode = mode)
...
parameterGrid2 <- dials::grid_regular(hidden_units = tune(),
                                      penalty = tune(),
                                      dropout = tune(),
                                      epochs = tune(),
                                      activation = tune(),
...
# parameter grid를 적용한 cross validation을 수행합니다
grid_search_result2 <- goophi::gridSerachCV(rec = rec,
                                           model = model,
                                           v = v,
                                           data = data_train,
                                           parameterGrid = parameterGrid2
)
grid_search_result2
