...
## RMSE plot
```{r}
extractRmseValues <- function(tuned_results_list) {
  rmse_df <- data.frame()
  model_name <- tuned_results_list

  for (i in seq(v + 1, v + v * iter, by = 1)) {
    rmse_value <- tuned_results$.metrics[[i]] %>%
    ...
    rmse_plot_data <- data.frame(
      model_name = "RF",
      mean_rmse = rmse_summary$mean_rmse,
      lower_bound = rmse_summary$lower_bound,
      upper_bound = rmse_summary$upper_bound
    )
  }
